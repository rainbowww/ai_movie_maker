...
- **무료 신경망 한국어 TTS는 이 환경에서 전부 막혔습니다.** 남은 후보를 전수 확인했고,
  우회하지 않았습니다. 다른 네트워크에서는 대부분 그대로 동작하므로, 로컬에서 돌린다면
  `edge-tts`가 가장 현실적인 0원 대안입니다(한국어 신경망 음성 다수 보유).

  | 후보 | 설치 | 호출 | 결과 |
  |---|---|---|---|
  | Gemini TTS | 가능 | — | 키 없음 (사용자 발급 대기) |
  | `edge-tts` (MS 신경망) | 성공 | `speech.platform.bing.com` | **조직 정책 403** (CONNECT 거부) |
  | `gTTS` (구글 번역) | 성공 | `translate.google.com` | **조직 정책 403** (CONNECT 거부) |
  | Piper | 성공 | — | 한국어 음성 모델 자체가 없음 |
  | HuggingFace 모델 | — | `huggingface.co` | 도메인 차단 |
