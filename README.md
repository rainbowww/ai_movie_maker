...
- **음성**: 엔진이 둘입니다. `--engine` 기본값은 `auto`로, **신경망(edge-tts)을 먼저
  시도하고 안 되면 espeak-ng으로 내려앉되 그 이유를 출력합니다.** 조용히 품질이 떨어진
  결과물을 모르고 편집하는 일이 없도록 한 것입니다.

  | 엔진 | 품질 | 비용 | 필요한 것 |
  |---|---|---|---|
  | `edge-tts` | 진짜 한국어 신경망 음성 (`ko-KR-InJoonNeural`/`ko-KR-SunHiNeural`) | **0원** | `speech.platform.bing.com` 접속 |
  | `espeak-ng` | 포먼트 합성음 | 0원 | 없음 (완전 오프라인) |

  espeak 쪽은 화자별 목소리·피치가 갈리고 EQ·컴프레서·룸·라우드니스 정규화를 거칩니다.
  사람 목소리는 아니지만 10분을 들을 수는 있습니다. 신경망 쪽에는 그 마스터링 체인을
  걸지 않습니다 — 3.4kHz 노치와 룸은 합성음의 쇳소리를 가리려고 넣은 것이라, 이미 깨끗한
  목소리에는 탁함만 더합니다. 레벨 정규화만 맞춰서 두 엔진이 섞여도 이어붙습니다.
- **발음**: 자막에 보이는 글자와 엔진에 넣는 글자를 분리합니다(`services/speech_text.py`).
  espeak-ng에는 실측되는 결함이 둘 있습니다 — `espeak-ng -v ko+m3 -x -q`로 확인됩니다.

  | 넣은 글자 | 음소 출력 | 문제 |
  |---|---|---|
  | `AI 영상` | `(en),eI'aI(ko) j'@NsaN` | **영어 보이스로 전환**됐다가 돌아옴 → 한 단어만 목소리가 바뀜 |
  | `90초` | `g'u_! s'ip_! tSh'o` | `_!`는 끊김. 숫자 자리마다 박혀서 "구/십/초" 세 토막 |

  그래서 자막은 `AI`·`100%` 그대로 두고, 읽히는 문장만 `에이아이`·`백 퍼센트`로
  바꿉니다. 수사는 계수사를 보고 한자어(`30분`→삼십 분)와 고유어(`3개`→세 개)를
  가릅니다. 현재 두 프로젝트 48장면 기준 **음소 결함 11건 → 0건**입니다.
  특정 장면만 다르게 읽히길 원하면 `Scene.speech_text`에 직접 적으면 그 값이 이깁니다.
  Gemini에는 두 결함이 없으므로 정규화를 건너뛰고 원문을 그대로 보냅니다.
- **한계**: 화면은 도식이고 목소리는 합성음입니다. 구조·타이밍·자막을 확인하는 용도입니다.

여기까지 마음에 들면 키를 넣고 품질만 올리면 됩니다. 렌더 단계는 그대로 재사용됩니다.

### 3-1. 키가 생겼을 때도 명령 하나 (유료)

```bash
python -m scripts.make_gemini_cut demo-solo --dry-run   # 호출 수·예상 비용만
python -m scripts.make_gemini_cut demo-solo             # 생성 + 렌더
python -m scripts.make_gemini_cut demo-solo --voice-only # 음성만 교체
```

`--dry-run`은 아무것도 호출하지 않고 이번 실행에 몇 장/몇 자가 들어가고 얼마쯤
나올지만 알려줍니다. 장면이 끝날 때마다 저장하므로 중간에 끊겨도 같은 명령을 다시
...
- **음성 엔진은 실측한 대로 갈립니다.** 이 저장소를 만든 컨테이너의 네트워크 정책을
  후보별로 직접 확인한 결과입니다.

  | 후보 | 호출 대상 | 이 컨테이너 | 비고 |
  |---|---|---|---|
  | **Gemini API** | `generativelanguage.googleapis.com` | **열림** | 키만 있으면 바로 동작. 키 발급은 브라우저에서 |
  | **`edge-tts`** (MS 신경망) | `speech.platform.bing.com` | 차단 | **0원 + 신경망.** 로컬에선 그대로 동작 |
  | `gTTS` (구글 번역) | `translate.google.com` | 차단 | |
  | Piper | — | 설치는 됨 | 한국어 음성 모델 자체가 없음 |
  | HuggingFace 모델 | `huggingface.co` | 차단 | |

  `aistudio.google.com`은 막혀 있지만 **키 발급은 브라우저에서 하는 일이라 상관없습니다.**
  API 호스트가 열려 있으므로 `make_gemini_cut`은 키를 넣는 즉시 동작합니다.

  차단을 풀고 싶으면 우회가 아니라 **환경의 네트워크 정책을 바꾸는 것이 정식 경로**입니다
  ([문서](https://code.claude.com/docs/en/claude-code-on-the-web)). 정책을 허용으로 바꾸면
  `--engine edge`가 그대로 통합니다.
