...
def build_voices(project, storage_root: Path, speed: int) -> int:
    scenes_dir = project.scenes_dir(storage_root)
    made = 0
    for scene in sorted(project.scenes, key=lambda s: s.order):
        ...
        scene.audio_path = str((scenes_dir / f"{scene.id}.wav").relative_to(storage_root))
        made += 1
    return made
...
def main() -> None:
    ...
    if not args.no_voice and not shutil.which("espeak-ng"):
        raise SystemExit("espeak-ng이 없습니다: sudo apt install espeak-ng (또는 --no-voice)")
    ...
    if args.no_voice:
        ...
        voices = build_voices(project, settings.storage_dir, args.speed)
    ...
    print()
    print(f"완성: {out_path}")
    print(f"  길이 {mins}분 {secs}초 · 화면 {images}개 · 음성 {voices}개 · 자막 {'번인' if not args.no_subtitles else '없음'}")
